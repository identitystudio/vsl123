...
export function ScriptInput({
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      setProgress(60);
      console.log('%c🧠 STEP 2.5: Analyzing Emotional Beats...', 'background: #9C27B0; color: white; fsize: 12px; padding: 2px 6px; border-radius: 4px;');

      // Step 2.5: Analyze Emotional Beats
      let emotionalBeats: any[] = [];
      try {
        const analyzeResponse = await fetch('/api/analyze-emotions', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ slides: styledSlides }),
          signal,
        });

        if (analyzeResponse.ok) {
           const analysisData = await analyzeResponse.json();
           emotionalBeats = analysisData.beats || [];
           console.log(`%c❤️ Emotional Beats Found: ${emotionalBeats.length}`, 'color: pink; font-weight: bold;');
           
           // Map beats to slides
           emotionalBeats.forEach((beat) => {
             beat.slideIds.forEach((slideId: string) => {
               const s = styledSlides.find((slide) => slide.id === slideId);
               if (s) {
                 s.emotionalBeat = beat.name;
                 s.emotion = beat.emotion; // Override basic emotion
                 s.visualPrompt = beat.visualPrompt;
                 s.videoPrompt = beat.videoPrompt;
                 s.imageKeyword = beat.visualPrompt; // Use visual prompt as keyword backup
               }
             });
           });
        }
      } catch (err) {
        console.error('Failed to analyze emotions:', err);
      }

      setProgress(70);

      // Step 3: Generate AI Images for each Emotional Beat (instead of stock photos)
      console.log(`%c🎨 GENERATING AI IMAGES for ${emotionalBeats.length} beats...`, 'background: #FF9800; color: white; font-size: 12px; padding: 2px 6px; border-radius: 4px;');
      
      const beatImages = new Map<string, string>(); // beatName -> imageUrl

      // Helper to generate image for a beat
      const generateForBeat = async (beat: any) => {
        try {
          console.log(`🎨 Generating for beat: "${beat.name}"...`);
          const res = await fetch('/api/generate-infographics', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
          ...
          console.error(`Failed to generate image for beat ${beat.name}`, e);
        }
        return null;
      };
      ...
      const chunkedBeats = [];
      for (let i = 0; i < emotionalBeats.length; i += 3) {
        chunkedBeats.push(emotionalBeats.slice(i, i + 3));
      }

      // Video generation has been moved to EmotionalBeatsSidebar to ensure persistence
      // and allow the user to continue working while videos generate in the background.

      // Assign generated images to slides
      styledSlides.forEach((slide) => {
        if (slide.emotionalBeat && beatImages.has(slide.emotionalBeat)) {
           const url = beatImages.get(slide.emotionalBeat)!;
           slide.hasBackgroundImage = true;
           slide.backgroundImage = {
             url,
             opacity: 60,
             blur: 0,
             displayMode: slide.style.background === 'split' ? 'split' : 'crisp',
           };
           slide.style.background = slide.style.background === 'split' ? 'split' : 'image';
        }
      });
      
      // Fallback: If any slides still missing images (not covered by beats or gen failed), use stock search
